...
## - tsg.sh ibonjour     - Installs Bonjour Network Protocol.
## - tsg.sh chkey        - Determine if you have appropriate keys. 
## - tsg.sh bupdate      - update packages and backup package file. 
## - tsg.sh setalias     - Sets up a git based development environment 
## - tsg.sh bu2usb       - Backup to USB
## - tsg.sh bufrusb      - Backup from USB
## - tsg.sh <date> <ext> - does something cool
## 		Where <date> = yyy/mm/dd 
## - tsg.sh gupdate      - updates your git repository. 
## - tsg.sh bookmarks    - Points bookmarks & fix keys for nautilus. 
## - tsg.sh dsply <file[doc,txt,pdf]> displays these three types
...
#
function chkey(){
#* function chkey - checks to see if you have appropriate keys, if not fixes this issue.
echo "This is the \""$FUNCNAME"\" function in "$0" version "$version #debug
echo "This is the command:  ls -alt ~/.ssh | grep rsa"
uroot
# echo $(ls -alt ~/.ssh | grep rsa)
ky=$(cd ~/.ssh |ls -1 | grep rsa)
echo "$ky"
# cd -
# spause
cd ~/.ssh
...
#
#
#https://www.ebay.com/sch/i.html?_fromsane=R40&_trksid=pmkdir -p /home/flint/.config/gtk-3.0/obe2334524.m570.l1313&_nkw=home+elevator+lift&_sacat=0&LH_TitleDesc=0&_osacat=0&_odkw=home+elevator+kit
#
#
...
echo "This is the \""$FUNCNAME"\" function in "$0" version "$version #debug
rm -rf need > /dev/null
for prog in openssh-server gparted nautilus pluma git snmp gedit most rsync iputils-arping  iputils-ping  iputils-tracepath
# git"less" "wget" 
#d ;do echo $prog; done
...
if [ "$#" -eq "1" ] && [ "$1" = "chkey"       ]; then ARGS="1"; fi
